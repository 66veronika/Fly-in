class Validator:
    ...
    def validate_zones(self) -> None:
        ...
        for zone in zones:
            ...
            if not name:
                raise ValueError(f"Line {line}: zone name cannot be empty")
            if "-" in name:
                raise ValueError(f"Line {line}: zone name cannot contain dashes ({name})")
            if name in names:
                raise ValueError(f"Line {line}: duplicate zone name ({name})")
            names.add(name)
            ...
            try:
                ...
                raise ValueError(f"Line {line}: zone coordinates must be integers")
            ...
            if zone["type"] == "start_hub":
                ...
                raise ValueError(f"Line {line}: invalid zone type ({zone['type']})")
    ...
    def validate_zone_metadata(self, zone: dict) -> None:
        ...
        for key in metadata:
            if key not in self.ALLOWED_ZONE_KEYS:
                raise ValueError(f"Line {line}: unknown zone metadata key '{key}'")

        zone_type = metadata.get("zone", "normal")
        if zone_type not in self.ALLOWED_ZONE_TYPES:
            raise ValueError(f"Line {line}: invalid zone type '{zone_type}'")
        zone["zone_type"] = zone_type  # ulož zpracovanou hodnotu pro pozdější použití

        raw_max_drones = metadata.get("max_drones", "1")
        try:
            max_drones = int(raw_max_drones)
        except ValueError:
            raise ValueError(f"Line {line}: max_drones must be an integer")
        if max_drones <= 0:
            raise ValueError(f"Line {line}: max_drones must be a positive integer")
        zone["max_drones"] = max_drones

        zone["color"] = metadata.get("color")  # None pokud nezadáno
    ...
    def validate_connections(self) -> None:
        ...
        for connection in self.data["connections"]:
            ...
            if not from_zone:
                raise ValueError(f"Line {line_number}: source zone cannot be empty")
            if not to_zone:
                raise ValueError(f"Line {line_number}: destination zone cannot be empty")

            # zóny definované PŘED touto connection (spec: "previously defined")
            known_zones = {
            ...
            if from_zone not in known_zones:
                raise ValueError(f"Line {line_number}: unknown zone ({from_zone})")
            if to_zone not in known_zones:
                raise ValueError(f"Line {line_number}: unknown zone ({to_zone})")
            if from_zone == to_zone:
                raise ValueError(f"Line {line_number}: a zone cannot connect to itself")
    ...
    def validate_connection_metadata(self, connection: dict) -> None:
        ...
        for key in metadata:
            if key not in self.ALLOWED_CONNECTION_KEYS:
                raise ValueError(f"Line {line}: unknown connection metadata key '{key}'")

        raw_capacity = metadata.get("max_link_capacity", "1")
        try:
            capacity = int(raw_capacity)
        except ValueError:
            raise ValueError(f"Line {line}: max_link_capacity must be an integer")
        if capacity <= 0:
            raise ValueError(f"Line {line}: max_link_capacity must be a positive integer")
